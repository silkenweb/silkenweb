...
pub trait Spacing: ElementBuilder {
    ...
    ///
    /// A `size` of `None` will set the margin to `auto`
    fn margin_at(self, size: Option<Size>, side: Side) -> Self {
        self.class([match size {
            None => match side {
    ...
    fn padding_at(self, size: Size, side: Side) -> Self {
        self.class([match (size, side) {
