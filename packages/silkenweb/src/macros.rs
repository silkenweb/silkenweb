...
/// });
///
/// trait DefaultElements {}
///
/// impl DefaultElements for Div {}
///
/// trait SlotElements {}
///
...
///
/// element_slot!(MyElement, default, my_default_slot, DefaultElements);
/// element_slot!(MyElement, "my-slot", my_slot, SlotElements);
///
...
macro_rules! element_slot {
    ($element:ty, default, $method_prefix:ident $(, $child_trait:path)? $(,)?) => {
        element_slot!(
            $element,
            ::std::option::Option::None as ::std::option::Option<String>,
            $method_prefix,
            $($child_trait)?
        );
    };
    ($element:ty, $slot:expr, $method_prefix:ident $(, $child_trait:path)? $(,)?) => {$crate::macros::paste!{
        impl<Dom: $crate::dom::Dom> [< $element:camel >] <Dom>
        {
            fn [< $method_prefix _ child >]<N> (
                self,
                child: impl $crate::value::SignalOrValue<Item = N>
            ) -> Self
            where
                N: $($child_trait +)? $crate::node::ChildNode<Dom>
            {
                Self(
                    $crate::node::element::ParentElement::child(
                        $crate::elements::HtmlElement::slot(self, $slot).0,
                        child
                    )
                )
            }

            fn [< $method_prefix _ optional_child >]<N>(
                self,
                child: impl $crate::value::SignalOrValue<Item = ::std::option::Option<N>>
            ) -> Self
            where
                N: $($child_trait +)? $crate::node::ChildNode<Dom>
            {
                Self(
                    $crate::node::element::ParentElement::optional_child(
                        $crate::elements::HtmlElement::slot(self, $slot).0,
                        child
                    )
                )
            ...
            fn [< $method_prefix _ children >]<N>(self, children: impl IntoIterator<Item = N>) -> Self
            where
                N: $($child_trait +)? Into<$crate::node::Node<Dom>>
            {
                Self(
                    $crate::node::element::ParentElement::children(
                        $crate::elements::HtmlElement::slot(self, $slot).0,
                        children
                    )
                )
            ...
            fn [< $method_prefix _ children_signal >]<N>(
                self,
                children: impl $crate::macros::SignalVec<Item = N> + 'static,
            ) -> Self
            where
            ...
            {
                Self(
                    $crate::node::element::ParentElement::children_signal(
                        $crate::elements::HtmlElement::slot(self, $slot).0,
                        children
                    )
                )
